...
/**
 * 🚀 Darul Umah School Management System - Auto Deployment Script
 * 
 * This script will automatically deploy your system to Render using their API
 * Run with: node deploy-to-render.js
 */
...
async function deployToRender() {
  log('\n🚀 Darul Umah School Management System - Auto Deployment', 'cyan');
  log('=' * 60, 'cyan');
  ...
  try {
    ...
    const dbResponse = await makeRequest(`${RENDER_API_BASE}/services`, {
      method: 'POST',
      headers,
      body: {
        name: 'darul-umah-db',
    ...
    const dbInfoResponse = await makeRequest(`${RENDER_API_BASE}/services/${dbId}`, {
      headers
    });
    ...
    const backendResponse = await makeRequest(`${RENDER_API_BASE}/services`, {
      method: 'POST',
      headers,
      body: {
    ...
    const frontendResponse = await makeRequest(`${RENDER_API_BASE}/services`, {
      method: 'POST',
      headers,
      body: {
    ...
    while ((!backendReady || !frontendReady) && attempts < maxAttempts) {
      ...
      if (!backendReady) {
        const backendStatus = await makeRequest(`${RENDER_API_BASE}/services/${backendId}`, { headers });
      ...
      if (!frontendReady) {
        const frontendStatus = await makeRequest(`${RENDER_API_BASE}/services/${frontendId}`, { headers });
